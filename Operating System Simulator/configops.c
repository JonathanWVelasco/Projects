...
Algorithm: utility function to support display
			of CPU scheduling or LogTo code strings
...
PostCondition: in correct operation,
               returns a pointer to correct configuration data structure
Exceptions: correct and appropriately (without program failure)
            responds to and reports filea access failure,
            incorrectly formatted lead or end descriptors,
            incorrectly forrmatted prompt, data out of range,
      ...
      if (getStringToDelimeter(fileAccessPtr, COLON, dataBuffer) != NO_ERR)
           {
			...
			//set end message to line capture failure
				// functionL: copyString
			copyString( endStateMsg, "Configuration state line capture error" );
               ...
               // assign to data pointer depending on config item
                ...
                switch( dataLineCode )
                    ...
                    case CFG_QUANT_CYCLES_CODE:
                        ...
                        tempData->quantumCycles = getCpuSchedCode( dataBuffer );
                        break;
...
PostCondition: returns True if data value is within specified parameters,
               False otherwise
    ...
    switch( lineCode )
       {
        ...
        case CFG_LOG_TO_CODE:
            ...
            // check for not finding one of log to strings
               // function: compareString
            if ( compareString( tempStr, "both" ) != STR_EQ
