...
Precondition: linked list pointer assigned to null or to onee op command link,
...
OpCodeType *clearMetaDataList( OpCodeType *localPtr )
   {
    // check for local pointer no set to null (list not empty)
    if( localPtr != NULL )
       {
...
Precondition: linked list, with or without data
              (shoudl not be called if no data)
...
Algorithm: get first three letters of input string to get command
Precondition: provide starting index
PostCondition: return command (ref param) and update index (return)
Exceptions: none
Notes: none
*/
...
PostCondition: return success (Boolean), returns linked list head via parameter,
...
Boolean getMetaData( char *fileName,
    ...
    *opCodeDataHead = NULL;
    ...
    if( getStringToDelimeter( fileAccessPtr, COLON, dataBuffer ) != NO_ERR
    ...
    if( accessResult == LAST_OPCMD_FOUND_MSG )
       {
        // check for start and end op cound counts equal
        if( startCount == endCount )
           {
            ...
            // check last line for incorrect end descriptor
               // function: getLineTo, compareString
            if( getStringToDelimeter( fileAccessPtr, PERIOD, dataBuffer) != NO_ERR
    ...
    // otherwise, assume didn't find end
		...
		// set end state message
			// function: copyString
		copyString( endStateMsg, "Corrupt metadata op code" );
...
int getOpCommand( FILE *filePtr, OpCodeType *inData )
   {
    ...
    // get whole op command as string
       // function: getLineTo
    accessResult = getStringToDelimeter(filePtr, SEMICOLON, strBuffer);
	...
	// set all strct values that may not be initialized to defaults
	inData->pid = 0;
...
Algorithm: skips leading white space,
			aquires next integer from op command input string
...
int getNumberArg( int *number, char *inputStr, int index )
	...
	*number = 0;
	...
	// end loop accross string length
...
Precondition: test value is a character
PostCondition: if test value is a value '0' < vlaue < '9',
...
Algorithm: check string argument for one of the allowed arguments
Precondition: input string is provided
PostCondition: in correct operation,
